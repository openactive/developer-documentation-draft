...
Error handling is similar to [Checkpoint 1](checkpoint-1.md#error-handling). There are however a few error `@types` that are not relevant in C1 that may be relevant here, for example `IncompleteAttendeeDetailsError`. Other than those edge cases, you can probably share a lot of the error handling code between the two endpoints.
